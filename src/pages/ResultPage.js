        ...
        <section className="guardian-result-card" aria-labelledby="guardian-result-title">
          ...
          <div className="metric-grid result-metric-grid">
            ...
            <article className={`metric ${hasMetricValue(yellowingIndex) ? 'measured' : 'pending'}`}>
              <span>황변 변화</span>
              <strong>{hasMetricValue(yellowingIndex) ? formatMetricValue(yellowingIndex) : '준비 중'}</strong>
              <span>{hasMetricValue(yellowingIndex) ? '/ 100' : '색상 분석 예정'}</span>
            </article>
            <article className={`metric ${hasMetricValue(gumInflammationIndex) ? 'measured' : 'pending'}`}>
              <span>잇몸 변화</span>
              <strong>{hasMetricValue(gumInflammationIndex) ? formatMetricValue(gumInflammationIndex) : '준비 중'}</strong>
              <span>{hasMetricValue(gumInflammationIndex) ? '/ 100' : '색상 분석 예정'}</span>
            </article>
          </div>
