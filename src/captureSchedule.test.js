...
test('만 7세부터 12세까지는 2주 주기를 적용한다', () => {
  ...
  expect(getCaptureSchedule(`${birthYear}-01-01`).scheduleLabel).toBe('매월 1일·15일');
});

test('생년월일이 없으면 기본 주 1회를 적용한다', () => {
  expect(getCaptureSchedule(null).scheduleLabel).toBe('매주 일요일');
});
...
test('만 13세 이상은 매월 1일 일정을 적용한다', () => {
  ...
  expect(getCaptureSchedule(`${birthYear}-01-01`).scheduleLabel).toBe('매월 1일');
});
