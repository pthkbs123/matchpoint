...
팀 `main`의 자녀별 3회 개인 색상 기준을 유지하면서 다음 보정 기능을 추가했습니다.

- `마이페이지 → 색상 분석 테스트`: 원본, WB+CLAHE, WB+Bilateral+CLAHE 비교
- LAB b/a와 HSV H/S/V 원시값, LAB·HSV 잇몸 보조점수 일치도 확인
- 기존 촬영 이력을 지우지 않는 자녀별 `기준 다시 만들기`
- 정상·황변·잇몸 붉음 참고 라벨과 전처리별 변동 폭 누적
- 정상/변화 사진이 각각 3장 이상일 때 `.env` 보정값 후보 계산
- 브라우저 로컬 저장 및 CSV 내보내기

실제 사진 검증 후 `backend/.env`의 `COLOR_YELLOW_LAB_B_*`,
`COLOR_GUM_LAB_A_*`, `COLOR_GUM_HSV_S_*` 값을 변경하면 코드 수정 없이
보정값을 적용할 수 있습니다. 이 결과는 개발 보정용이며 의료 진단값이 아닙니다.
